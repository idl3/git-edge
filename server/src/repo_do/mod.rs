...
/// A17 quota defaults for the small-disposable-app profile (ROADMAP P1 #9): bite abuse
...
/// A18: pushes per minute per credential per repo (GE_RATE_PUSHES_PER_MIN). Abuse
/// damping only — the heavy hammer is a zone-level Cloudflare rate-limit rule.
const DEFAULT_PUSHES_PER_MIN: i64 = 30;

/// A19: memoized advertisement state, valid for exactly one refs_version. `ls` holds a
...
struct BeginDto {
    push_id: String,
    principal: String,
    /// sha1 of the presented token — the A18 rate-limit bucket key.
    #[serde(default)]
    key: String,
}
#[derive(serde::Deserialize)]
...
impl DurableObject for RepoDo {
    ...
    async fn fetch(&self, mut req: Request) -> worker::Result<Response> {
        ...
        // A17: `owner!<owner>` registry DOs never boot as repos — no meta, no jobs,
...
/// The /_do/refs DTO body — built once per refs_version and memoized (A19).
...
impl RepoDo {
    ...
    /// A17: `owner!<owner>` registry routes — the cross-repo quota oracle. These DOs
    ...
    fn debug_state(&self) -> Result<Response, Error> {
        ...
        json(serde_json::json!({
            ...
            // A19 memo counters — hits/(hits+misses) is the advertisement-rebuild
    ...
    /// A19: refs snapshot memoized on refs_version. A push is the only writer and
    ...
    fn push_begin(&self, meta: &Meta, b: &BeginDto) -> Result<Response, Error> {
        self.rate_check(&b.key)?; // A18: before the row exists — throttled pushes hold nothing
        ...
        // A17: a repo counts against GE_QUOTA_MAX_REPOS_PER_OWNER until its first
    ...
    /// A18 sliding-window throttle: two fixed 60 s counter buckets in `rate`; the
    ...
    pub fn commit_push(&self, req: &CommitRequest) -> Result<CommitResponse, Error> {
        ...
        if let Some(pack) = &req.pack_id {
            // A17: per-repo object/byte quotas, evaluated now that ingest has posted
    ...
    /// A17 quota accounting: sum of packs.count/bytes over live packs plus this
